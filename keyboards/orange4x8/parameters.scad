...
mcu_row_spacing = 15.6;
mcu_row_count = 2;  // Unused
mcu_pin_count = 18;
mcu_pin_pitch = 2.54;
mcu_pin_offset = 1;  // Offset from the rear of the PCB
mcu_connector_width = 9;  // Width of the connector (for plate cutout)
mcu_connector_length = 8;  // Distance the connector extends onto the MCU (for plate cutout)
mcu_connector_height = 4;  // Height of the plug housing
mcu_connector_offset = 1.5; // Vertical offset of plug center from PCB center
mcu_pcb_thickness = 1;
mcu_pcb_offset = 1; // Vertical offset of MCU from PCB (useful for traditional PCBs)
mcu_socket_width = mcu_width+4;
mcu_socket_length = mcu_length+4;
expose_mcu = true; // Opens up cutout grid above MCU footprint

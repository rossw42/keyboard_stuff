...
from src.geometry.solid_models_lp import generate_top_frame_solid, export_step, export_stl
...
def main():
    ...
    print(f"  - External dimensions: {CASE_LENGTH}mm x {CASE_WIDTH}mm x {TOP_FRAME_HEIGHT}mm")
    print(f"  - PCB opening: {PCB_OPENING_LENGTH}mm x {PCB_OPENING_WIDTH}mm (centered)")
    print(f"  - USB cutout: {USB_CUTOUT_WIDTH}mm x {USB_CUTOUT_HEIGHT}mm (centered on top edge)")
    print(f"  - Brass insert holes: 6 locations, {BRASS_INSERT_DIAMETER}mm dia, {BRASS_INSERT_DEPTH}mm deep")

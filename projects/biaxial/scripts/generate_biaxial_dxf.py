...
# Push each half outward toward the plate edges (widens the center gap)
EDGE_SHIFT = 8.0  # mm each half moves toward its edge
LEFT_COL_X  = [x - EDGE_SHIFT for x in LEFT_COL_X]
RIGHT_COL_X = [x + EDGE_SHIFT for x in RIGHT_COL_X]
...
# Center space keys: 1.25u wide caps, raised to almost in line with the home
# row (2nd-to-bottom): 0.25u below the home-row centerline.
SPACE_CAP_U = 1.25                       # keycap width in units
SPACE_SEP_EXTRA = 3.5                    # mm gap allowance between the two caps
SPACE_SEP = SPACE_CAP_U * UNIT + SPACE_SEP_EXTRA  # center-to-center distance
SPACE_ROW_OFFSET = -0.75 * UNIT          # 0.25u below home row (-0.5u)


def center_spaces():
    """Two 1.25u space keys side by side in the center gap, raised to almost
    in line with the home row. Left space rotated -6° (matching the left
    clusters), right space +6° (matching the right clusters). Each rotates
    about its own center."""
    gap_cx = (LEFT_COL_X[-1] + RIGHT_COL_X[0]) / 2
    cy = COL_CENTER_Y + SPACE_ROW_OFFSET
    return [
        (gap_cx - SPACE_SEP / 2, cy, -ANGLE_RAD),  # left space
        (gap_cx + SPACE_SEP / 2, cy, +ANGLE_RAD),  # right space
    ]
...
def main():
    ...
    # --- Accurate keycap clearance (polygon min distance between caps) ---
    def cap_corners(cx, cy, angle_rad, half_w=UNIT / 2, half_h=UNIT / 2):
        pts = [(-half_w, -half_h), (half_w, -half_h), (half_w, half_h), (-half_w, half_h)]
        c, s = math.cos(angle_rad), math.sin(angle_rad)
        return [(cx + dx * c - dy * s, cy + dx * s + dy * c) for dx, dy in pts]
    ...
    def poly_gap(poly_a, poly_b):
        best = float('inf')
        for i in range(4):
            for j in range(4):
                best = min(best, seg_dist(poly_a[i], poly_a[(i + 1) % 4],
                                          poly_b[j], poly_b[(j + 1) % 4]))
        return best

    sp = center_spaces()
    (lsx, lsy, lsa), (rsx, rsy, rsa) = sp
    _sw = SPACE_CAP_U * UNIT / 2
    cap_l = cap_corners(lsx, lsy, lsa, half_w=_sw)
    cap_r = cap_corners(rsx, rsy, rsa, half_w=_sw)
    # Nearest inner-column keys: home-row and bottom-row of each inner column
    inner_l_keys = column_keys(LEFT_COL_X[-1], -ANGLE_RAD)[2:4]
    inner_r_keys = column_keys(RIGHT_COL_X[0], +ANGLE_RAD)[2:4]
    l_gap = min(poly_gap(cap_l, cap_corners(*k)) for k in inner_l_keys)
    r_gap = min(poly_gap(cap_r, cap_corners(*k)) for k in inner_r_keys)
    print(f"\nCenter spaces (2x {SPACE_CAP_U}u side by side, rotated ±{ANGLE_DEG}° to match clusters):")
    print(f"  Left  space: center ({lsx:.3f}, {lsy:.3f})  angle=-{ANGLE_DEG}°")
    print(f"  Right space: center ({rsx:.3f}, {rsy:.3f})  angle=+{ANGLE_DEG}°")
    print(f"  Keycap gap between spaces:        {poly_gap(cap_l, cap_r):.2f}mm")
    print(f"  Keycap gap to inner-left column:  {l_gap:.2f}mm")
    print(f"  Keycap gap to inner-right column: {r_gap:.2f}mm")

    print(f"\nTotal keys: {len(keys)} ({len(keys)-2} alpha + 2 spaces)")

    # Check bounds
    violations = check_bounds(keys)
    if violations:
        print(f"\nWARNING: {len(violations)} boundary violations:")
        for v in violations:
            print(f"  {v}")
    else:
        print("OK: All cutout corners within plate body bounds")
    ...
    key_names = left_key_names + right_key_names + ["SpaceL", "SpaceR"]

...
class Key:
    ...
    # Marked via Layout.mark_converters() / kbforge --converter-keys:
    # this position takes a PG1350->PG1425 switch-converter adapter
    # (15x15mm 3D-printed carrier on a PG1425 "Choc X" PCB footprint)
    # instead of a normal plate-mounted switch.
...
class Layout:
    ...
    @property
    def converters(self) -> List[Key]:
        """Switch keys marked as switch-converter (PG1425 adapter) positions."""
        return [k for k in self.switches if k.converter]

    def mark_converters(self, specs: List[str]) -> int:
        """
        Mark switch keys as converter positions. Each spec is one of:
          * "all"          — every switch key
          * "rXcY"         — matrix position (e.g. "r0c3"); requires matrix
          * any other text — case-insensitive match against the key's labels
        Returns the number of keys marked. Unknown specs are ignored
        (callers can compare the count against expectations).
        """
        if not specs:
            return 0
        ...
        lowered = [s.strip().lower() for s in specs if s and s.strip()]
        if "all" in lowered:
        ...
        label_specs = set()
        for spec in lowered:
            m = _re.fullmatch(r"r(\d+)c(\d+)", spec)
            if m:
                matrix_specs.add((int(m.group(1)), int(m.group(2))))
            else:
                label_specs.add(spec)

        marked = 0
        for key in self.switches:
            hit = (key.matrix_row, key.matrix_col) in matrix_specs
            if not hit and label_specs:
                labels = {l.strip().lower() for l in key.labels if l}
                hit = bool(labels & label_specs)
            if hit and not key.converter:
                key.converter = True
                marked += 1
        return marked
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "matrix_rows": rows,
            "matrix_cols": cols,
            "converter_keys": len(self.converters),

...
def _ensure_kbforge_footprints(
    build_dir: Path,
    config_text: str,
    quiet: bool = False,
) -> None:
    """
    Stage kbforge's own custom footprints (kbforge/footprints/*.js, e.g.
    switch_pg1425 for switch-converter positions) into the build's
    footprints/ folder when the config references any of them.
    """
    source = Path(__file__).resolve().parent.parent / "footprints"
    if not source.is_dir():
        return
    for js in sorted(source.glob("*.js")):
        if js.stem not in config_text:
            continue
        dest = build_dir / "footprints" / js.name
        dest.parent.mkdir(parents=True, exist_ok=True)
        if not quiet:
            print(f"  staging custom footprint {js.name} ...", flush=True)
...
def _ensure_footprints(
    ...
    _ensure_kbforge_footprints(build_dir, config_text, quiet)

...
function activate(context) {
    console.log('🚀 Ergogen DXF Viewer v2.0.0 activating...');
    
    // Create output channel for logging
    outputChannel = vscode.window.createOutputChannel('Ergogen DXF Viewer');
    outputChannel.appendLine('Extension activated');
    
    // Create status bar item
    statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    statusBarItem.text = '$(eye) DXF';
    statusBarItem.tooltip = 'Ergogen DXF Viewer';
    statusBarItem.command = 'ergogen-dxf-viewer.openViewer';
    ...
    vscode.window.showInformationMessage('🎹 Ergogen DXF Viewer ready!');
}
...
function registerCommands(context) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('ergogen-dxf-viewer.openViewer', () => {
    ...
    // Run Ergogen command
    context.subscriptions.push(
        vscode.commands.registerCommand('ergogen-dxf-viewer.runErgogen', async () => {
            await runErgogen();
        })
    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('ergogen-dxf-viewer.refreshViewer', () => {
            refreshViewer();
        })
    );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('ergogen-dxf-viewer.fitToWindow', () => {
            if (currentPanel) {
...
function registerEventListeners(context) {
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration(e => {
            if (e.affectsConfiguration('ergogen-dxf-viewer')) {
                if (currentPanel) {
...
function updateWebviewConfiguration() {
    ...
    currentPanel.webview.postMessage({
        ...
        config: {
            ...
            showDimensions: config.get('showDimensions')
        }
    });
}

...
from data_models.ergogen_point import PointsCollection, ErgogenPoint
...
class ErgogenYAMLGenerator:
    ...
    def _generate_base_yaml(
        ...
        if self._is_regular_grid(points_collection):
            ...
            # Each point becomes its own zone (Ergogen requires points to live
            # under points.zones; a zone with just an anchor yields one point
            # named after the zone).
            config = {
                'points': {
                    'zones': self._generate_absolute_points(points_collection)
                }
            }
    ...
    def _generate_absolute_points(self, points_collection: PointsCollection) -> Dict[str, Any]:
        ...
        for point in points_collection.points:
            ...
            # Add rotation if present
            if abs(point.rotation) > 0.1:
                point_config['anchor']['rotate'] = round(point.rotation, 1)
    ...
    def validate_yaml(self, yaml_content: str) -> List[str]:
        ...
        try:
            ...
            zones = points.get('zones')
            if not isinstance(zones, dict) or not zones:
                errors.append("Points section must contain a non-empty 'zones' dictionary")
                return errors
            
            # Validate each zone
            for zone_name, zone_data in zones.items():
                if not isinstance(zone_data, dict):
                    errors.append(f"Zone '{zone_name}' must be a dictionary")
                    continue
                
                # A zone must position its point(s): either an anchor with a
                # shift, or rows/columns definitions (grid layout)
                anchor = zone_data.get('anchor')
                has_anchor_shift = (
                    isinstance(anchor, dict)
                    and isinstance(anchor.get('shift'), list)
                    and len(anchor['shift']) == 2
                )
                has_grid = 'rows' in zone_data or 'columns' in zone_data
                if not (has_anchor_shift or has_grid):
                    errors.append(f"Zone '{zone_name}' missing position (anchor.shift or rows/columns)")
                
                # Validate anchor shift values are numeric
                if has_anchor_shift:
                    for value in anchor['shift']:
                        if not isinstance(value, (int, float)):
                            errors.append(f"Zone '{zone_name}' anchor shift values must be numeric")
                            break

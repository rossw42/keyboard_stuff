...
# Add this directory to the Python path so the local packages resolve
# regardless of where the CLI is invoked from
sys.path.insert(0, str(Path(__file__).parent))

try:
    from parsers.kle_to_ergogen_converter import (
        KLEToErgogenConverter,
        KLEToErgogenError,
        convert_kle_file
    )
    from generators.ergogen_yaml_generator import (
